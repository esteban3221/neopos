...
private:
    ...
    std::shared_ptr<SQLite> db = std::make_shared<SQLite>();
    std::unique_ptr<conf> Conf = std::make_unique<conf>(db);
    std::unique_ptr<reglas> Reg = std::make_unique<reglas>(db);
...
protected:
    //Gtk::Window *window_conf_ticket, *window_regla_venta;
    Gtk::Box *box_pos;
    ...
    Gtk::Button *btn_pago_efectivo, *btn_pago_tarjeta, btn_add_articulo_popover, *btn_add_piezas, *btn_add_regla;
...
public:
    ...
    ~Pos()
    {
        ...
        m_MenuPopup_main.unparent();
        popover_ingreso_articulos.unparent();
        popover_precio_articulo.unparent();
        popover_cantidad_articulo.unparent();
    }
};
