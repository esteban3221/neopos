...
Pos::Pos(const Glib::RefPtr<Gtk::Application> &app) : builder(std::make_unique<Builder>(
                  ...
                  "<object class=\"GtkBox\">"
                    ...
                    "<child>"
                      "<object class=\"GtkBox\">"
                        ...
                        "<property name=\"hexpand-set\">true</property>"
                        "<child>"
                          "<object class=\"GtkButton\" id=\"btn_add_regla\">"
                            "<property name=\"css-classes\">warning</property>"
                            "<property name=\"icon-name\">guides-symbolic</property>"
                            "<property name=\"tooltip-text\">Añade Regla.</property>"
                          "</object>"
                        "</child>"
                        "<child>"
...
"</interface>"))
{
    ...
    Conf->set_modal(true);
    Conf->set_hide_on_close(true);
    Conf->set_default_size(900, 500);
    Conf->set_resizable(false);

    Reg->set_transient_for(*this);
    Reg->set_modal(true);
    Reg->set_hide_on_close(true);
    Reg->set_default_size(900, 500);
    Reg->set_resizable(false);
...
void Pos::cargar_glade()
{
    ...
    btn_add_regla = builder->m_refBuilder->get_widget<Gtk::Button>("btn_add_regla");
...
void Pos::carga_señales()
{
    ...
    btn_add_regla->signal_clicked().connect([this]()
                                            { Reg->show(); });
